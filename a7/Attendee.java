...
public class Attendee implements Comparable<Attendee>{
	...
	public int compareTo(Attendee other) {
		if (this.eventTicket.compareTo(other.eventTicket) == 0
				&&this.arrivalTime.compareTo(other.arrivalTime) == 0){
			return 0;
		}else
			if (this.eventTicket.compareTo(other.eventTicket) < 0){
				return -1;
			}else if (this.eventTicket.compareTo(other.eventTicket) > 0) {
				return 1;
			}else{
				if(this.arrivalTime.compareTo(other.arrivalTime) > 0){
					return 1;
				}else{
					return -1;
				}
